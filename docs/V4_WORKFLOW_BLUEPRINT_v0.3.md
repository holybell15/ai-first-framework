# V4 Workflow Blueprint v0.3

> **Status**: Draft — 待凱子 Final Review
> **Base**: v0.2 (Opus) + Codex Review 4 項修正
> **Date**: 2026-03-31
> **Changelog**:
> - v0.1 — Codex 初版骨架
> - v0.2 — Opus 補充（Superpowers 分析、Skill 分層、CLAUDE.md 瘦身、遷移策略）
> - v0.3 — 採納 Codex Review：恢復 Task-Master dispatcher、恢復 specialist role matrix、恢復 artifact registry、獨立 planning discipline
...
2. **攔截（Intercept）**— 偵測 blocked / scope drift / design drift / test escalation，決定退回哪一層
3. **交接（Handoff）**— 管理 specialist 之間的交接，確保 handoff protocol 被執行

**Task-Master 不做的事**：
- 不載入專案的完整技術 context（只讀 STATE + TASKS）
- 不做實作、不寫 code、不做設計
- 不做 Gate 審查（那是 Review specialist 的事）
- 不取代 specialist 的專業判斷

**Task-Master 的 context 限制**：
- 必載：STATE.md (~200 tokens) + TASKS.md (任務清單部分)
- 可載：project-config.yaml（判斷啟用了哪些 skill）
- 不載：SRS、API Spec、Schema、任何正式文件內容
...
- Review specialist 有特殊的 gate 審查權限，QA 沒有
...
| **planning-with-tasks** | 執行過程中的進度追蹤 | **v0.3 修正**：從 v0.2 的「合併至 writing-plans」改為獨立保留。重新命名自 planning-with-files，專注於 TASKS.md 的狀態追蹤和進度管理 |
...
|---|---|---|
| **何時用** | 任務開始前 | 任務執行中 |
| **做什麼** | 把 Feature 拆解成 2-5 分鐘的原子任務，寫出具體的 code / 指令 / 路徑 | 追蹤 TASKS.md 中每個任務的狀態、更新進度、記錄 findings |
| **產出** | 可執行的實作計畫 | 更新後的 TASKS.md |
| **為什麼不合併** | Plan（思考如何做）和 Track（記錄做到哪）是不同的認知活動。合併後容易「一邊想一邊追蹤」，結果兩邊都做不好 |
...
### 攔截條件（必須回 Task-Master）
- blocked / needs_discussion / handoff
- scope drift / design drift 偵測
- test escalation（測試顯示上游有誤）
- specialist 完成當前任務
...
| Verify | QA, Security, DevOps, Review | context-roles/GROUP_Verify.md |
...
│   └── GROUP_Verify.md              # QA + Security + DevOps + Review
│
...
| context-seeds/ (13 files) | context-roles/ (3 Group files) | 合併為 Group + specialist 區塊 |
...
4. 遷移角色：
   - 13 個 SEED → 3 個 GROUP file（保留 specialist 差異）
5. 遷移 skill：
...
| planning-with-files | T1 | **重新命名**為 planning-with-tasks | 專注進度追蹤 |
...
*v0.3 — Codex review corrections applied*
*Pending: 凱子 final review → v1.0 定稿*
