...
## Completion Status Protocol（強制）

> 靈感來源：gstack — 結構化完成狀態，防止 Agent 靜默失敗。

每個 Agent 完成時，**必須**在交接摘要中標記以下四種狀態之一：

| 狀態 | 含義 | 後續行為 |
|------|------|---------|
| `✅ DONE` | 全部完成，有證據支持 | 正常交接下一個 Agent |
| `⚠️ DONE_WITH_CONCERNS` | 完成但有疑慮 | 交接 + 列出 concerns，下游 Agent 需注意 |
| `🔴 BLOCKED` | 無法繼續，已嘗試但失敗 | 停止 + 列出已嘗試方案 + 建議解法 |
| `❓ NEEDS_CONTEXT` | 缺少資訊無法判斷 | 停止 + 精確描述需要什麼資訊 |
...
BLOCKED 報告格式：
  ...
  ATTEMPTED:
    ...
    2. [方法 B] → [失敗原因]
    3. [方法 C] → [失敗原因]
  RECOMMENDATION: [建議的解決方向]
  ESCALATE_TO: [建議由誰處理：PM / Architect / 人工]
```
...
```markdown
| **完成狀態** | ✅ DONE / ⚠️ DONE_WITH_CONCERNS / 🔴 BLOCKED / ❓ NEEDS_CONTEXT |
```

---

## AskUserQuestion 標準格式（強制）

> 靈感來源：gstack AskUserQuestion re-grounding — 每次提問都重述 context，防止用戶迷失。

Agent 向用戶提問時，**必須**遵循以下 5 段格式：

```markdown
## 📌 需要您的決定

**目前進度**：[正在做什麼 Pipeline / Agent / Feature]
**此問題背景**：[為什麼需要問這個，1-2 句白話]

**選項**：
  (A) [選項描述] — 工時估算 👤 ~[人工時間] / 🤖 ~[AI 時間]
  (B) [選項描述] — 工時估算 👤 ~[人工時間] / 🤖 ~[AI 時間]
  (C) [其他 / 自訂]

**推薦**：(A)，原因：[一句話]
**完成度**：如果選 (A)，目前完成度 [N]/10

請選擇：
```

### 規則
1. **一次只問一個決策** — 不要批量問 3 個無關的問題
2. **必須給推薦** — 不要問「你覺得呢？」而不給方向
3. **白話文** — 16 歲高中生能看懂的用語
4. **工時對比** — 讓用戶知道 AI 做 vs 人工做的成本差異（Boil the Lake 依據）

---

## Friction Logging + Analytics（每次交接自動記錄）

### Friction Log（選填但鼓勵）

工作過程中遇到任何摩擦（流程不順、文件不清、skill 缺陷），在交接摘要末尾加一行：

```markdown
<!-- FRICTION: [skill/步驟] | [什麼卡住了] | [改善建議] -->
```

Pipeline-orchestrator 會自動提取並追加到 `memory/friction_log.md`。
不強制、不阻塞，但每次 Retro 會彙整這些摩擦點作為框架改善輸入。

### Skill Analytics（自動記錄）

每個 Agent 完成交接後，pipeline-orchestrator 自動追加一行到 `memory/analytics.jsonl`：

```jsonl
{"ts":"[ISO時間]","skill":"[使用的skill]","agent":"[Agent名]","feature":"[F碼]","duration_s":[秒],"outcome":"[success/blocked/skipped]"}
```

Agent 不需要手動寫這行 — orchestrator 從交接摘要的 Completion Status 自動推算。
