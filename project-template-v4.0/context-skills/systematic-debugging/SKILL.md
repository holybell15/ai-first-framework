...
> 靈感來源：gstack /investigate — 3 次假設失敗後強制升級。
...
強制升級報告：
  ...
  ATTEMPTED:
    ...
    H2: [假設] → [測試方法] → [失敗原因]
    H3: [假設] → [測試方法] → [失敗原因]
  REQUEST: 需要 [Architect / DBA / 人工] 協助
  SCOPE_LOCK: 啟用 destructive-guard freeze mode，鎖定在 [目錄]
```

**3-Strike 後的行為**：
1. 報告 BLOCKED 狀態（Completion Status Protocol）
2. 啟用 Freeze Mode 鎖定修改範圍（防止亂改）
3. 等待人工或其他 Agent 介入
4. 不要嘗試第 4 個假設（你已經證明你不了解根因）

---

## 已知 Bug Pattern 資料庫

> 靈感來源：gstack /investigate — 已知 bug 簽名快速匹配。

Phase 2 生成假設前，先比對已知 pattern：

| Pattern ID | 簽名 | 典型根因 | 快速驗證 |
|-----------|------|---------|---------|
| `RACE-01` | 間歇性失敗、多執行緒環境 | Race condition | 加 sleep / 鎖定後是否消失 |
| `NIL-01` | `TypeError: Cannot read property of null` | Null propagation | 追蹤 null 來源鏈 |
| `STATE-01` | 「昨天還好好的」、重啟就修復 | Stale cache / state corruption | 清 cache 後測試 |
| `CONFIG-01` | 在 A 環境正常、B 環境失敗 | 環境變數差異 | diff .env 檔案 |
| `POOL-01` | 超時、ECONNREFUSED、連接被拒 | 連接池耗盡 | 檢查 pool 使用量 |
| `SERIAL-01` | JSON parse 錯誤、型別不匹配 | 序列化/反序列化不一致 | 比對 request/response raw data |
| `ASYNC-01` | 資料偶爾遺失、順序錯亂 | Promise 未 await / callback hell | 搜尋 missing await |
| `MIGRATE-01` | 特定欄位 NULL、Schema mismatch | Migration 未跑或跑錯順序 | 檢查 migration 狀態表 |

**使用方式**：Phase 1 收集症狀後，先掃描 Pattern 資料庫，匹配到 → 直接跳 Phase 2 用該 pattern 的快速驗證
