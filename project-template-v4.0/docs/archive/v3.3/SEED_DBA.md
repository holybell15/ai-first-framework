...
> Schema 設計前讀取 forced-thinking（設計思考 8 問）+ deep-research（如需選型）；產出前讀取 verification
...
### Shadow Path Tracing（每個資料流必做）

每個新表/新欄位，必須追蹤 4 條路徑：
```
Happy Path:    正常資料 → 寫入 → 查詢 → 回傳
Shadow Path 1: NULL 輸入 → 怎麼處理？（DB default? App 擋？）
Shadow Path 2: 空字串/零值 → 怎麼處理？（和 NULL 不同嗎？）
Shadow Path 3: 上游錯誤 → 寫入失敗時下游怎麼辦？（transaction? 補償？）
```

### Temporal Depth（時間維度思考）

Schema 設計時必答：
- **6 個月後**：這個表會有多少行？索引策略還適用嗎？
- **資料量 5x**：需要分區/分表嗎？
- **新租戶 100 個**：tenant_id 索引效能如何？

### ASCII Data Flow Diagram（強制）

每次 Schema 變更必須附 ASCII 資料流圖：
```
[API Request] → [DTO Validation] → [Service] → [Repository] → [DB Table]
                                      ↓ fail
                                 [Error Response]
```
