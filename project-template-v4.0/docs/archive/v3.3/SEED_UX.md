...
> 靈感來源：gstack /plan-design-review — 12 個設計師思維直覺。

### 12 Cognitive Patterns（內化直覺）

1. **看系統不看畫面** — 每個設計決策問「這對整體系統有什麼影響」
2. **同理心即模擬** — 扮演不同角色走一遍流程（新手 / 老手 / 管理者）
3. **層級即服務** — 視覺層級服務於使用者的注意力分配
4. **邊界案例偏執** — 47 字元的名字、0 筆結果、斷網、第一次 vs 第 100 次
5. **減法優先** — 先問「能不能拿掉」再問「要不要加上」
6. **一致性紀律** — 同樣的操作在全系統有同樣的視覺語言
7. **空狀態即機會** — 空畫面不是錯誤，是引導使用者的好時機
8. **動態勝過靜態** — 載入中、過渡、微互動都是設計的一部分
9. **數據驅動布局** — 有 3 筆和 300 筆的畫面不應該長一樣
10. **可及性即品質** — WCAG 不是加分題，是必答題
11. **錯誤狀態即信任** — 錯誤畫面的品質 = 使用者對產品的信任度
12. **移動端先行** — 先在 375px 寬設計，再擴展到桌面

### Interaction State 強制覆蓋（每個畫面必答）

| 狀態 | 必須定義 |
|------|---------|
| Empty | 沒有資料時顯示什麼 |
| Loading | 載入中的畫面（skeleton / spinner / 漸進式） |
| Error | 發生錯誤時顯示什麼 + 使用者能做什麼 |
| Overflow | 資料超過預期時（名字太長、列表太多） |
| First-time | 第一次使用 vs 第 100 次使用的差異 |
| Permission | 無權限時的畫面（不是空白，是有意義的提示） |

**Gate 1 Review 時，Review Agent 會檢查每個畫面是否都有以上 6 個狀態的定義。**
