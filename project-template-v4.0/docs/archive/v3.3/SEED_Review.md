...
▌ Gate 2 — 技術可行性（Tier 2 或 3）
  ...
  G2-CC 🟡 若為 Call Center 功能：agent state / interaction state 未混淆；事件來源、recording linkage、routing 規則、KPI-sensitive timestamps 已文件化
...
AFL 驗證（workflow_rules.md §36）：
  ...
  G3-CC 🟡 若為 Call Center 功能：routing / transfer / hold / wrap-up / recording / duplicate event 至少有對應測試證據或明確排除說明
...
═══════════════════════════════════════
【Cross-Model 雙重驗證（Second Opinion）】
═══════════════════════════════════════

> 靈感來源：gstack /codex — 用不同 AI 模型作為獨立第二意見，防止單一模型盲點。

▌ 自動觸發規則（非可選 — 符合條件時強制執行）

  以下情境自動觸發 Cross-Model Review，Review Agent 不得跳過：
  🔴 Gate 3 中涉及安全或合規的檢查項（G3-03, G3-04, G3-07）
  🔴 G4-ENG 資料契約驗證（DC-04~DC-06）
  🔴 Hotfix 快速審查（HF-01~HF-03）
  🟡 Gate 2 架構決策涉及跨模組重構
  🟡 審查中發現分歧（主審查不確定是 🟢 還是 🟡）

  可選觸發（Review Agent 判斷）：
  - 技術選型時想聽第二意見
  - 高風險架構決策（跨模組重構、資料結構變動）

▌ 三種模式

  Review 模式 → 獨立審查，產出 pass/fail 判定
    使用場景：Gate 3 安全審查、API Spec 一致性驗證、Contract Test
    指令：「請用 Cross-Model Review 模式審查 [文件]」
    執行方式：將待審查文件交給不同模型，不提供主審結果（防止偏見）

  Adversarial 模式 → 對抗性審查，主動找漏洞和反例
    使用場景：架構 ADR 壓力測試、效能假設驗證、安全威脅模型
    指令：「請用 Cross-Model Adversarial 模式挑戰 [設計決策]」
    自動觸發：diff 超過 500 行時，Review 自動升級為 Adversarial
    執行方式：要求模型扮演攻擊者/懷疑者，找出最弱的假設

  Consult 模式 → 自由諮詢，取得不同觀點
    使用場景：技術選型時想聽第二意見、設計權衡取捨
    指令：「請用 Cross-Model Consult 模式討論 [議題]」

▌ 執行流程

  1. Review Agent 準備審查摘要 + 關鍵文件片段
  2. 選擇 Second Opinion 模型（依可用性和任務複雜度）：
     - Claude Sonnet（快速、適合 Review/Consult 模式）
     - Claude Haiku（最快、適合格式檢查和 Contract Test）
     - Claude Opus（深度分析、適合 Adversarial 模式）
     選擇策略：Review 模式用不同於主審的模型等級
  3. 將摘要交給 Second Opinion 模型
     ⚠️ Review 模式：不提供主審結果（獨立判斷）
     ⚠️ Adversarial 模式：提供主審結果（要求反駁）
  4. 收集回饋，與自身審查結果比對
  5. 差異處標記為 🔍 需人工確認
  6. 一致的判定可提升整體信心度

▌ 輸出格式（附加在 Gate Review 報告末尾）

  ```
  ## 🔍 Cross-Model Review
  觸發類型：[自動/手動]
  模式：[Review/Adversarial/Consult]
  主審模型：[使用的模型]
  Second Opinion 模型：[使用的模型]

  ### 獨立判定結果
  | 檢查項 | 主審判定 | Second Opinion | 一致性 |
  |--------|---------|---------------|--------|
  | [項目] | 🟢/🟡/🔴 | 🟢/🟡/🔴 | ✅ 一致 / 🔍 分歧 |

  ### 分歧分析（僅列出不一致項）
  - [差異項 1]：主審認為 🟢，Second Opinion 認為 🟡
    Second Opinion 理由：[...]
    解決方式：[採納主審/採納 Second Opinion/升級為人工確認]

  ### 結論
  一致率：[N]%
  最終調整：[列出因 Second Opinion 而調整的判定]
  ```

▌ 規則

  - Cross-Model Review 不取代 Review Agent 的判定權
  - 最終燈號由 Review Agent 決定
  - 一致率 < 70% → 強烈建議升級為人工 Review
  - Adversarial 模式發現的問題必須逐一回應（接受或駁回+理由）

═══════════════════════════════════════
【Scope Drift Detection（範圍偏移偵測）】
═══════════════════════════════════════

Gate Review 時，除了品質檢查，還必須確認「做的是不是當初說的」：

  Step 1：從 TASKS.md / PR 描述 / RS 提取「應該做什麼」
  Step 2：從 git diff 提取「實際做了什麼」
  Step 3：比對，分類：
    ✅ DONE — 計劃中且已實作
    🟡 PARTIAL — 計劃中但只做了一部分
    ❌ NOT DONE — 計劃中但未實作
    🔄 CHANGED — 計劃外的新增修改（scope creep）

  → CHANGED 項目必須解釋原因，否則標記 🟡
  → NOT DONE 項目必須有明確計畫（下個 Sprint / 已移除）

═══════════════════════════════════════
【Plan Completion Audit（計畫完成度審計）】
═══════════════════════════════════════

  ```
  COMPLETION: 5/7 DONE, 1 PARTIAL, 1 NOT DONE
    ✅ F02-API-01  POST /calls          → 已實作
    ✅ F02-API-02  GET /calls/:id       → 已實作
    ✅ F02-API-03  PUT /calls/:id       → 已實作
    ✅ F02-DB-01   calls table          → 已建立
    ✅ F02-FE-01   CallPopup.vue        → 已實作
    🟡 F02-FE-02   CallHistory.vue      → 只做了列表，缺少篩選
    ❌ F02-TC-03   edge case: 無 CRM    → 未寫測試
  ```

═══════════════════════════════════════
【Kill Criteria Check（放棄條件檢查）】
═══════════════════════════════════════

  Gate Review 時，必須檢查 RS 中的 Kill Criteria 是否已觸發：

  □ Feature Appetite 超時？ → RS 中定義的 Appetite 時間 × 150% 是否已到？
  □ Gate 退回次數？ → 本 Feature 累計退回 ≥ 3 次？
  □ 外部依賴？ → RS 中的依賴截止日是否已過？

  任一觸發 → 🔴 標記「Kill Criteria Triggered」
  → 提出選項：(A) 縮小 scope (B) 延長 Appetite (C) 停止開發
  → 用 AskUserQuestion 格式提問，由人類決定

═══════════════════════════════════════
【Two-Pass Review（兩階段審查）】
═══════════════════════════════════════

  Pass 1 — CRITICAL（先做，阻塞項）：
    SQL injection / race condition / tenant 洩漏 / enum 不一致 / 未處理 error
    → 發現 → 立即標記 🔴

  Pass 2 — INFORMATIONAL（後做，不阻塞但記錄）：
    magic number / dead code / 缺少測試 / 命名不一致 / 效能疑慮
    → 發現 → 標記 🟡，不影響 Gate 燈號
...
═══════════════════════════════════════
  ...
  AUTO-FIX（直接修，不問人）：
    ...
    - console.log 殘留
    - 明顯的 typo
    → 修完後一行摘要：「已自動修正 3 項：[清單]」

  ASK（批量提問，一次問完）：
    - 業務邏輯判斷
    - 架構選型
    - 設計方向
    → 收集所有 ASK 項目，用 AskUserQuestion 格式一次提問
