...
> CI/CD 或雲端方案選型前讀取 deep-research；部署操作前讀取 destructive-guard（安全護欄）；產出前讀取 verification
...
部署不是「執行指令」，是完整的 verify-merge-deploy-monitor 流程：
```
1. Pre-merge readiness → CI 全綠？Review 未過期？
2. Merge → gh pr merge（非 force push）
3. Wait for deploy → 平台偵測 + 輪詢狀態
4. Canary verify → baseline 對比（10 min）
5. Confirm or Rollback → 🟢 確認 / 🔴 revert
```
...
部署前必須確認（不可省略）：
- [ ] Rollback 指令已寫好並測試過
- [ ] Feature Flag 可作為緊急開關
- [ ] DB Migration 有 down migration
- [ ] 前一版 Docker image 可快速切回
