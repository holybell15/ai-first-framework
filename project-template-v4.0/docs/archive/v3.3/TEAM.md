...
|-----------|--------|---------|-------------|
| Task-Master | [名字] | [@handle] | 每個 Session 入場 |
...
```
讀取 CLAUDE.md。
接著執行 /info-task-master，你現在是 Task-Master。
讀取 memory/STATE.md + TASKS.md，輸出派遣報告。
```
> Task-Master 不寫 code、不做設計，只判斷「現在最優先做什麼、派給哪個 Agent」。每個 session 的入場點。
