...
已有 Codebase 的專案首次導入框架。不強迫補全舊 code 所有文件，只建立 baseline 快照，接入後的新工作才走完整 Pipeline。
...
| 標準差距評估 | Review | GAP 分析報告 |
| 環境 + CI 整合 | DevOps | `.github/workflows/` + `.env.example` |

接入後，修改現有功能走**簡化四步法**（理解現況 → 補最小 RS → 補測試 → 執行驗證），大幅重構才走完整 Pipeline。
...
| 嚴重度 | 條件 | 路徑 |
|--------|------|------|
| 🔴 Critical / 🟠 High | 服務中斷、資料錯誤、核心功能失效 | → Hotfix Pipeline |
...
| 部署 | DevOps | — |
| 補件（RS + Gate 文件）| PM + Review | 48hr 內 |
